...
<section class="section">
  ...
  <div class="card">
    ...
    <div class="card-body">
      ...
      {{-- LOOPIN UNTUK CRITERIA DAN SUBCRITERIA --}}
      @foreach ($criterias as $criteria)
      <div class="col-12 col-lg-6">
        <div class="form-group">
          <label for="criteria_id" name="criteria_id">{{ $criteria->nama }}</label>

          <select class="form-control @error('subcriteria_id') is-invalid @enderror" name="subcriteria_id[]">
          <option value="">--Pilih--</option>
          @foreach ($criteria->subcriterias as $subcriteria)
              <option value="">{{ $subcriteria->namass }}</option>
          @endforeach
          </select>
        </div>
